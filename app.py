...
class CustomPDF(FPDF):
    def header(self):
        ...
        self.cell(200, 10, "Export Management System", ln=True, align="C")
        self.ln(10)  # Line break

    def footer(self):
        self.set_y(-15)
        self.set_font("Arial", "I", 10)
        self.cell(0, 10, "Generated by Export Management System", align="C")
...
def view_cart():
    ...
    # ✅ FIX: Ensure total is calculated dynamically
...
def payment():
    ...
    if request.method == "POST":
        # ✅ FIX: Update `total_amount` field in `orders` table
        cursor.execute("UPDATE orders SET status = 'Confirmed', total_amount = ? WHERE id = ?", (total_price, order_id))
...
def view_orders():
    ...
    # ✅ FIX: Dynamically calculate total amount instead of using `orders.total_amount`
    cursor.execute('''
        SELECT o.id, SUM(oi.quantity * oi.price) AS total_amount, o.status
        FROM orders o
        JOIN order_items oi ON o.id = oi.order_id
        WHERE o.vendor = ?
